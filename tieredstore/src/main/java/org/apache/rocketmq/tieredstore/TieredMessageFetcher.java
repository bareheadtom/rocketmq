...
import org.apache.rocketmq.tieredstore.file.IndexItem;
...
public class TieredMessageFetcher implements MessageStoreFetcher {
    ...
    public CompletableFuture<QueryMessageResult> queryMessageAsync(
        ...
        CompletableFuture<List<IndexItem>> future = tieredIndexService.queryAsync(topic, key, begin, end);
        ...
        return future.thenCompose(indexItemList -> {
            ...
            for (IndexItem indexItem : indexItemList) {
                ...
                CompletableFuture<Void> getMessageFuture = flatFile.getCommitLogAsync(indexItem.getOffset(), indexItem.getSize())
                    .thenAccept(messageBuffer -> result.addMessage(
                        new SelectMappedBufferResult(0, messageBuffer, indexItem.getSize(), null)));
